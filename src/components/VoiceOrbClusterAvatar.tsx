...
/* -------------------------------------------------------------------------- */
/*  Default coach portrait (embedded). Override via the avatarSrc prop.        */
/* -------------------------------------------------------------------------- */
const DEFAULT_AVATAR = "/coach-avatar.jpg";
...
export interface VoiceOrbClusterAvatarProps {
  /**
   * Current conversation state.
   * "idle"             — slow rotating cluster behind the avatar.
   ...
   */
  speaker?: Speaker;
  /** Coach photo URL shown in front, circle-masked. Defaults to the embedded portrait. */
  avatarSrc?: string;
  ...
  idleColor?: string;
  /** Overall square size in px (stage). Default 320. */
  size?: number;
  /** Diameter of the circular avatar photo in px. Default 115.
   *  The particle cluster sphere is rendered at 1.15× this diameter. */
  avatarSize?: number;
...
interface Particle {
  ...
  sz: number;  // per-particle size scalar: 0.4 (tiny) → 1.0 (current max)
}
...
export default function VoiceOrbClusterAvatar({
  speaker = "idle",
  ...
  // Mirror props into refs from an effect (this project's lint forbids
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      arr.push({
        ...
        rjy: Math.random() * 2 - 1,
        rjz: Math.random() * 2 - 1,
        sz: 0.4 + Math.random() * 0.6,  // varied sizes: 0.4× (tiny) to 1.0× (current max)
      });
    }
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      const C = S / 2;
      // cluster sphere radius = 1.15 × avatar radius → cluster diameter is 1.15× the avatar
      const ORB = (avatarSizeRef.current / 2) * 1.15;
      ...
      const proj: { sx: number; sy: number; rz: number; ry: number; sz: number }[] = [];
      ...
      for (let k = 0; k < proj.length; k++) {
        ...
        // multiply base radius by per-particle sz so sizes vary across the cluster
        const rad   = Math.max((0.7 + (q.rz + 1) * 0.45) * small * (S / 150) * q.sz, 0.4);
  ...
  return (
    ...
    >
      {/* particle cluster — behind the avatar (z 1) */}
      <canvas
        ref={canvasRef}
        style={{ position: "absolute", inset: 0, width: size, height: size, zIndex: 1 }}
      />
      {/* coach avatar — circle-masked, in front (z 2) */}
      <img
        src={avatarSrc}
        alt="Coach"
        style={{
